...
func _ready() -> void:
	...
	self.child_window = preload("res://Function/Weather/Weather.tscn").instance()
...
#	开启事件
	if Input.is_action_just_pressed("control") and child_window == null and state != State.STOP:
		self.child_window = preload("res://Function/Expression/Expression.tscn").instance().new("ballgame")
	match self.state:
		State.STOP:
			pass
		State.DRAGGING:
...
func set_mood(new_mood) -> void:
	...
	if self.emotion != pre_emotion:
		self.child_window = preload("res://Function/Expression/Expression.tscn").instance().new(self.emotion)
